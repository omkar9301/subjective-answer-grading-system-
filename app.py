...
def view_exam_answers(paper_id):
    ...
    for question in questions:
        ...
        student_text = student_answer.answer if student_answer else ""
        grade_percentage = grade_answer(question.text, student_text, ideal_answer)
        score = round((grade_percentage * question.max_marks) / 100, 2)

        graded_answers.append((question, student_text, ideal_answer, grade_percentage, score))
...
def teacher_dashboard():
    ...
    # Fetch question papers created by the teacher
    question_papers = QuestionPaper.query.filter_by(teacher_id=teacher_id).all()

    # Count active papers by this teacher
    active_papers = QuestionPaper.query.filter_by(teacher_id=teacher_id, is_active=True).count()

    # Count all students
    total_students = Student.query.count()

    # Handle activation/deactivation of papers
    if request.method == 'POST':
        paper_id = request.form.get('paper_id')
        action = request.form.get('action')
...
def create_question_paper():
    ...
    if request.method == 'POST':
        ...
        marks = request.form.getlist('marks[]')  # NEW

        for i, question_text in enumerate(questions):
            question = Question(text=question_text, paper_id=question_paper.id, max_marks=float(marks[i]))
...
def view_student_exams(student_id):
    ...
    # Get distinct exam_paper_ids from student's submitted answers
    submitted_paper_ids = db.session.query(ExamAnswer.exam_paper_id)\
        .filter_by(student_id=student.id)\
        .distinct().all()

    paper_ids = [id for (id,) in submitted_paper_ids]

    # Fetch actual submitted papers
    submitted_papers = QuestionPaper.query.filter(QuestionPaper.id.in_(paper_ids)).all()

    return render_template('view_student_exams.html', student=student, submitted_papers=submitted_papers)
...
def view_exam_answers_teachers(paper_id, student_id):
    ...
    for question in questions:
        ...
        ideal_answer = "; ".join([ans.text for ans in question.answers])
        grade_percentage = grade_answer(question.text, student_answer, ideal_answer)
        score = round((grade_percentage * question.max_marks) / 100, 2)

        graded_answers.append((question, student_answer, ideal_answer, grade_percentage, score))
        total_obtained += score
        total_max += question.max_marks
